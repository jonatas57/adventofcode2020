...
type State = M.Map (Int, Int, Int, Int) Bool

getState :: [String] -> State
getState ss = M.fromList [(cs, c == '#') | (cs, c) <- coords]
  where
    rows   = zip [0..] ss
    coords = concat $ map (\(x, cs) -> [((x, y, 0, 0), c) | (y, c) <- zip [0..] cs]) rows

getAdj :: (Int, Int, Int, Int) -> [(Int, Int, Int, Int)]
getAdj p@(x, y, z, w) = filter (/= p) [(x + i, y + j, z + k, w + l) | i <- [-1..1], j <- [-1..1], k <- [-1..1], l <- [-1..1]]
